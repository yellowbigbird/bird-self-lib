...
struct game {
	struct step *root_step;

//	struct list_head all_step;
	int step_nr;

	struct step **hash_bucket;
...
static void dump_step(const struct step *step)
{
	int i, j, n;
	char buf[10];
	const struct phase *phase; 
...
static inline int search_card_number(const struct phase *phase, char card)
{
	int i, j;
	int nr = 0;
	for (i = 0; i < 4; i++) 
...
static int check_phase(const struct phase *phase)
{
	int i, j, n;
	char f;
...
static inline char is_space_full(const struct phase *phase)
{
...
static inline char is_space_empty(const struct phase *phase)
{
	assert(phase);
	return phase->space[0];
}

static inline int get_space_count(const struct phase *phase)
{
	if (phase->space[3])
		return 0;
...
static inline char get_space(const struct phase *phase, int space_id)
{
	assert(phase && space_id < 4 && space_id >= 0);
	return phase->space[space_id];
}

static inline char set_space(struct phase *phase, int space_id, char card)
{
	assert(phase && space_id < 4 && space_id >= 0);
	if (card)
		assert_card(card);
...
static inline void insert_card_to_space(struct phase *phase, char card)
{
	char s;
	int i;
...
static inline void remove_card_from_space(struct phase *phase, int space_id)
{
	int n;
...
static inline char get_final(const struct phase *phase, int type)
{
	assert(phase && type >= 1 && type <= 4);
	return get_digital(phase->final[type - 1]);
}

static inline void increase_final(struct phase *phase, int type)
{
	assert(phase && type >= 1 && type <= 4);
	phase->final[type - 1]++;
}
...
static inline void erase_end_card(struct phase *phase, int cid)
{
//	int i;
//	char card;
...
static inline int get_end_card_pos(const struct phase *phase, int cid)
{
//	int i;
//	char card;
...
static inline char get_card_by_pos(const struct phase *phase, int cid, int pos)
{
	assert(phase && cid <= 7 && cid >= 0 && pos >= 0 && pos <= phase->pos[cid]);
	return phase->column[cid][pos];
}

static inline char get_end_card(const struct phase *phase, int cid)
{
//	int i;
//	char card;
...
static inline char cut_end_card(struct phase *phase, int cid)
{
//	int i;
	char card;
...
static inline void append_card_to_column(struct phase *phase, int cid, char card)
{
	assert(phase && cid <= 7 && cid >= 0);
	assert_card(card);
...
static inline int find_card_in_columns(const struct phase *phase, char card, int *cid, int *pos)
{
	int i, j;
...
static inline int find_card_in_space(struct phase *phase, char card)
{
	int i;
...
static inline int compare_phase(const struct phase *left, const struct phase *right)
{
	// TODO
	assert(left && right);
...
static inline int check_done(struct step *step)
{
	struct phase *p = &step->phase;
...
static inline int get_empty_column_count(const struct phase *phase)
{
	int i, nr = 0;
	for (i = 0; i < 8; i++) {
...
static inline int get_card_move_ability(struct phase *phase)
{
...
static inline void calc_step_hash_value(struct step *step)
{
	assert(step);
	step->hash = jhash(&step->phase, sizeof(step->phase) / sizeof(u32), 0x12345678);
}

static inline void insert_step_to_hash_bucket(struct step *sub, struct game *game)
{
	int n;
	assert(sub && game);
...
static inline struct step *clone_step_from_phase(const struct phase *phase, struct game *game)
{
	struct step *step;
	...
	if (step) {
		memcpy (&step->phase, phase, sizeof(*phase));

		INIT_LIST_HEAD(&step->sub);
		INIT_LIST_HEAD(&step->sub_list_head);
//		INIT_LIST_HEAD(&step->list);

		step->state = ST_INIT;
		step->level = 0;
		step->id = game->step_nr;
		step->hash_next = NULL;
		step->up_step = NULL;
...
static inline void release_step(struct step *step, struct game *game)
{
	int n;
	struct step *s, **prev;
...
static struct step *check_same_step(struct step *step, struct game *game)
{
	struct step *s;
	int n;
...
static inline int add_sub_level_step(struct step *node, struct step *sub, struct game *game)
{
	assert(node && sub);
...
static void move_card_from_column_to_space(struct phase *phase, int col_id)
{
	char c;
...
static void move_card_from_space_to_column(struct phase *phase, int space_id, int cid)
{
	char c;
#ifndef fast_move
	int n;
...
static void move_card_from_space_to_final(struct phase *phase, int space_id)
{
	char c;
	assert(phase && space_id < 4 && space_id >= 0);
	c = get_space(phase, space_id);
...
static void move_cards_between_column(struct phase *phase, int from_id, int to_id, int number)
{
	int i, n, m;
	char c;
#ifndef fast_move
...
static inline int step1_direct_to_final(struct step *step, struct game *game)
{
	struct phase *p;
	struct step *s;
	char t, c[2];
...
static inline int step3_force_move_card_to_final(struct step *step, struct game *game)
{
	int i, j, sum = 0;
	struct phase *p;
	char c, f;
...
int do_freecell_solve(struct game *game)
{
	int sum;
	struct step *s;
...
//	INIT_LIST_HEAD(&game->all_step);
	...
	game->hash_bucket = malloc(sizeof(struct step *) * MAX_HASH_BUCKET);
	assert(game->hash_bucket);
	memset(game->hash_bucket, 0, sizeof(struct step *) * MAX_HASH_BUCKET);
