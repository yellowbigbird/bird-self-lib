...
#define BRACKET(left, right)\
	...
	#endif
			//TODO get rid of the c_str and use an iterator to take advantage of shared strings
				...
				case JSON_TEXT('/'):  //a C comment
					if (*(++p) == JSON_TEXT('*')){  //a multiline comment
					   ...
					   while ((*(++p) != JSON_TEXT('*')) || (*(p + 1) != JSON_TEXT('/'))){
						  JSON_ASSERT_SAFE(*p, JSON_TEXT("Null terminator inside of a multiline quote"), COMMENT_DELIMITER(); goto endofrunner;);
						  *runner++ = *p;
					   }
					   ++p;
					   COMMENT_DELIMITER();
			 ...
			 case JSON_TEXT('\"'):  //a quote
				*runner++ = JSON_TEXT('\"');
				while(*(++p) != JSON_TEXT('\"')){  //find the end of the quotation, as white space is preserved within it
					JSON_ASSERT_SAFE(*p, JSON_TEXT("Null terminator inside of a quotation"), goto endofrunner;);
					switch(*p){
					   case JSON_TEXT('\\'):
						  *runner++ = JSON_TEXT('\\');
						  if (escapeQuotes){
 ...
 */
    ...
    void JSONWorker::UTF(const json_char * & pos, json_string & result) json_nothrow {
