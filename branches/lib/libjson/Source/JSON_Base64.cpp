...
json_string JSONBase64::json_encode64(const unsigned char * binary, size_t bytes) json_nothrow {
    ...
    for (size_t i = 0, aligned((bytes - misaligned) / 3); i < aligned; ++i){
		HitScopeCoverage(json_encode64, forloop);
    ...
    if (json_likely(misaligned != 0)){
		HitScopeCoverage(json_encode64, misaligned);
	   //copy the rest into a temporary buffer
	   unsigned char temp[3];
	   for (unsigned int i = 0; i < misaligned; ++i){
		   HitScopeCoverage(json_encode64, misaligned_loop_1);
		  temp[i] = *binary++;
	   }
	   for (unsigned int i = (unsigned int)misaligned; i < 3; ++i){
		   HitScopeCoverage(json_encode64, misaligned_loop_2);
		  temp[i] = '\0';
	   }
	   ...
	   if (misaligned == 2){
		   HitScopeCoverage(json_encode64, misaligned_equals_two);
		  result += chars64[((temp[1] & 0x0F) << 2) + ((temp[2] & 0xC0) >> 6)];
		  result += JSON_TEXT('=');
	   } else {
		   HitScopeCoverage(json_encode64, misaligned_not_two);
		  result += JSON_TEXT("==");
	   }
    } else {
		HitScopeCoverage(json_encode64, notmisaligned);
	}
...
std::string JSONBase64::json_decode64(const json_string & encoded) json_nothrow {
    const size_t length = encoded.length();
	   ...
	   if (json_unlikely(pos != json_string::npos)){
		  ...
		  if (pos != length - 1){
			 ...
			 HitScopeCoverage(json_decode64, pos_lengthminusone);
		  }
	   } else {
		   HitScopeCoverage(json_decode64, pos_is_npos);
	   }
    #endif
    ...
    if (json_likely(aligned != 0)){
	   ...
	   if (runner[2] != '='){  //not two = pads
		   HitScopeCoverage(json_decode64, not_two_pads);
		  const json_char third = toBinary(runner[2]);
		  result += ((second & 0xf) << 4) + ((third & 0x3c) >> 2);
		  if (runner[3] != '='){  //no padding
			  HitScopeCoverage(json_decode64, no_padding);
			 result += ((third & 0x3) << 6) + toBinary(runner[3]);
		  } else {
			HitScopeCoverage(json_decode64, padding_three);
		  }
	   } else {
			HitScopeCoverage(json_decode64, padding_two);
	   }
    } else {
		HitScopeCoverage(json_decode64, aligned_zero);
	}
    return result;
}
