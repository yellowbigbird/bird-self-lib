...
void SplitFile(HWND hProgressBar,TCHAR *szFullFileName,TCHAR *szOutputDirectory,UINT uSplitSize)
{
	...
	hInputFile = CreateFile(szFullFileName,GENERIC_READ,
	...
	SendMessage(hProgressBar,PBM_SETRANGE,0,(LPARAM)(lFileSize.QuadPart / uSplitSize));

	lRunningSplitSize.QuadPart = 0;

	pszBuffer = (TCHAR *)malloc(uSplitSize * sizeof(TCHAR));
	...
	{
		ReadFile(hInputFile,(LPVOID)pszBuffer,uSplitSize,&dwNumberOfBytesRead,NULL);

		StringCchPrintf(szOutputFile,MAX_PATH,_T("%s\\%s.%d"),szOutputDirectory,szFileName,nSplitsMade);
		...
		SendMessage(hProgressBar,PBM_SETPOS,nSplitsMade,0);
...
void MergeFiles(list<MergedFile_t> *MergedFileNameList,TCHAR *szOutputFileName)
{
	...
	hOutputFile = CreateFile(szOutputFileName,GENERIC_WRITE,
	0,NULL,CREATE_NEW,FILE_ATTRIBUTE_NORMAL,NULL);

	for(itr = MergedFileNameList->begin();itr != MergedFileNameList->end();itr++)
	{
		...
		if(hMergeFile != INVALID_HANDLE_VALUE)
		{
			...
			pszMergeBuffer = (TCHAR *)realloc(pszMergeBuffer,lMergeFileSize.LowPart * sizeof(TCHAR));

			if(pszMergeBuffer != NULL)
			{
				ReadFile(hMergeFile,(LPVOID)pszMergeBuffer,lMergeFileSize.LowPart,
