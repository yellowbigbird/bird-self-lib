...
{
    const wstring wstr = CConfig::Get().GetRootPath();
    if(wstr.length() < 1)
        return;
...
{
    const wstring wstrRootPath = CConfig::Get().GetRootPath();
...
{   
	...
	CFileInfo entry;
	bool ifok = false;	
	const wstring wstrRoot = CConfig::Get().GetRootPath();
